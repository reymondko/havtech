...
<div class="col-md-12">
    <div class="row">
        <div class="col-md-9">
            <h1 class="form-title">Add Event Photos (Step {{$step}})</h1>
        </div>
    ...
    @if(count($data['eventdata'])>0)
        <div class="row">
            @section ('pane1_panel_title', 'Uploaded Photos')
            @section ('pane1_panel_body')
                <div class="col-md-12">
                    @foreach ($data['eventdata'] as $key => $es)
                        <div class="photo-preview"  id="photo{{$key}}">
                        <div class="photo-image">
                                <img data-dz-thumbnail="" alt="{{$es['original_name']}}" src="/uploads/photos/{{$es['resized_name']}}" style="width:120px">
                            </div>
                            <button type="button"  onclick="deletePhoto('{{$es['original_name']}}','photo{{$key}}')" class="tn btn-default btn-outline deleteEventItem photo-remove"><i class="fa fa-minus-circle"></i> Delete photo</a></button>
                        </div>
                    @endforeach
                </div>
            @endsection
    ...
    <div class="row">
        ...
        <div class="row">
            ...
            <div class="col-md-2">
                <button onclick="$('#eventphotosform').submit()" id="nextBtn" class="btn btn-block btn-blue">Create Event</button>
            </div>
        </div>
        <form name="eventphotosform" id="eventphotosform">
            <input type="hidden" name="event_id" id="event_id" value="{{$event->id}}" > 
