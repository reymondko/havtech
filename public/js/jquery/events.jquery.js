$(document).ready(function() {
    ...
    /*
    remove dates auto appender in step 1
...
function sendAttendees(){
    if($("input[name='attendee[]']:checked").length > 0)
    {
...
function readURL(input,target) {
    if (input.files && input.files[0]) {
      ...
      reader.onload = function(e) {var targetz= target.includes("#thumb-preview");
        if(targetz){
            var addremove='';
        }
        else{
...
function deleteEvent(id){
    var prompt = window.confirm("Are you sure you want to Cancel this Event?");
    if(prompt){
...
function goBack(){
    ...
    $.ajax({
        url: '/events/go-back',
        type: 'POST',  
        data: {step:  $("#step").val(), _token: $('meta[name="_token"]').val(),event_id: $("#event_id").val()},
        success: function (data) { 
            if(data.success){
                window.location.replace("/events/step/"+data.step+"of" +data.total+"/create-event-"+data.url+"/"+data.event_id);
            }               
...
function cancelEvent(id){
    var prompt = window.confirm("Are you sure you want to Cancel this Event? \n(This will delete the entire event)");
    if(prompt){
        window.location.href = "/events/deleteevents?event_id="+id;
    }
}
